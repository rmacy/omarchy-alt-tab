...
const source = readFileSync(new URL("../BindingScript.js", import.meta.url), "utf8")
  .replace(/^\.pragma library\s*/m, "")
const bindingScript = {}
vm.createContext(bindingScript)
vm.runInContext(source, bindingScript, { filename: "BindingScript.js" })

const OWNER = "bitr0t.window-switcher-1756213000000-a1b2c3"
...
test("interpolates only a fixed inventory of validated Lua string literals", () => {
  const allowed = new Set([
    OWNER,
    "bitr0t-window-switcher",
    "bitr0t_window_switcher_owner",
    "bitr0t_window_switcher_active",
    "bitr0t_window_switcher_binds",
    "omarchy-shell -q shell call bitr0t.window-switcher ",
    "reset",
    "table",
    "userdata",
    "oneshot",
    ...
    "Window switcher: cancel",
    "Window switcher: select"
  ])
...
test("reapply retires exact tracked handles before creating new binds", () => {
  ...
  const registry = apply.indexOf("_G.bitr0t_window_switcher_binds = {}")
  const ownerSet = apply.indexOf("_G.bitr0t_window_switcher_owner = owner")
...
test("emitted shell methods are exactly advance, commit and cancel", () => {
  ...
  assert.match(apply, /pcall\(hl\.exec_cmd, "omarchy-shell -q shell call bitr0t\.window-switcher "/)
...
test("cleanup disables tracked handles and clears state without side effects", () => {
  ...
  assert.match(cleanup, /_G\.bitr0t_window_switcher_binds = nil/)
  const ownerGate = cleanup.indexOf("if _G.bitr0t_window_switcher_owner == owner then")
  const ownerClear = cleanup.indexOf("_G.bitr0t_window_switcher_owner = nil")
