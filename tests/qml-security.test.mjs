...
test("window discovery uses the bounded native Hyprland model", () => {
  assert.match(qml, /WindowModel\.switchableClients\(Hyprland\.toplevels/)
  assert.match(qml, /WindowModel\.MAX_CLIENTS/)
...
test("class fallback icons pass through the strict model sanitizer", () => {
  assert.match(qml, /WindowModel\.safeIconName\(fallbackClass\)/)
})
