...
// Pure generator for the compositor-side Hyprland Lua binding script.
// No Qt imports and no ambient state: every dynamic value arrives as a
// validated option, so generated Lua never interpolates client data and
// identical options always produce an identical script.

var SUBMAP = "bitr0t-window-switcher"
var SHELL_TARGET = "bitr0t.window-switcher"
var IPC_COMMAND = "omarchy-shell -q shell call bitr0t.window-switcher "

// Bounded registration retry policy consumed by Service.qml.
...
function bindingOptions(raw) {
  ...
  if (typeof options.ownerToken !== "string")
    throw new Error("BindingScript: ownerToken must be a string")
  var ownerToken = options.ownerToken
  if (!OWNER_TOKEN_PATTERN.test(ownerToken)) {
    throw new Error("BindingScript: ownerToken must be alphanumeric with . _ or - separators")
  }
  return {
...
function generateApply(raw) {
  ...
  return [
    ...
    "  return _G.bitr0t_window_switcher_owner == owner",
    "end",
    "local function live()",
    "  return owned() and _G.bitr0t_window_switcher_active == true",
    "end",
    ...
    "local previous = _G.bitr0t_window_switcher_binds",
    "if type(previous) == \"table\" then",
    "  for index = 1, #previous do",
    "    disable(previous[index])",
    "  end",
    "end",
    "_G.bitr0t_window_switcher_binds = {}",
    "local binds = _G.bitr0t_window_switcher_binds",
    ...
    "  local was_active = _G.bitr0t_window_switcher_active == true",
    "  if was_active and apply_pending then flush_delta() end",
    "  stop()",
    "  if was_active and method then",
    "    hl.timer(function()",
    "      if owned() then shell_call(method, \"ignored\") end",
    ...
    "    if not owned() then return end",
    "    push_advance(1)",
    "  end, { description = \"Window switcher: next\" }))",
    "  track(hl.bind(\"ALT + SHIFT + TAB\", function()",
    "    if not owned() then return end",
    "    push_advance(-1)",
    "  end, { description = \"Window switcher: previous\" }))",
    ...
    "    if not owned() then return end",
    "    settle(\"cancel\", false)",
    "  end, { description = \"Window switcher: cancel\" }))",
    "  track(hl.bind(\"RETURN\", function()",
    "    if not owned() then return end",
    "    settle(\"commit\", true)",
    "  end, { description = \"Window switcher: select\" }))",
    "end)",
    "track(hl.bind(\"ALT + TAB\", function()",
    "  if not owned() then return end",
    "  begin(1)",
    ...
    "  if not owned() then return end",
    "  begin(-1)",
    "end, { description = \"Window switcher (reverse)\" }))"
  ].join("\n")
}
...
function generateCleanup(raw) {
  ...
  return [
    ...
    "  end",
    "end",
    "_G.bitr0t_window_switcher_binds = nil",
